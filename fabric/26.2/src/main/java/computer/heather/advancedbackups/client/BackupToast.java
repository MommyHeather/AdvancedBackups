...
public class BackupToast implements Toast {
    ...
    @Override
    public void extractRenderState(GuiGraphicsExtractor context, Font renderer, long startTime) {
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, 0, 0, 0F, (float) (ClientConfigManager.darkMode.get() ? 0 : this.height()), this.width(), this.height(), this.width(), this.height() * 2);

        context.item(stack, 8, 8);
        ...
        if (!exists) {
            if (title.equals(I18n.get("advancedbackups.backup_finished"))){
                ...
                context.text(renderer, I18n.get(title), 25, 11, textColour, false);
                context.fill(4, 28, 156, 29, ColourHelper.colour
        ...
        context.text(renderer, title, 25, 11, textColour, false);
